namespace NServiceBus_6
{
    ...
    class AssemblyPublisherSource : IPublisherSource
    {
        ...
        public IEnumerable<PublisherTableEntry> GenerateWithBestPracticeEnforcement(Conventions conventions)
        {
            ...
            if (!entries.Any())
            {
                throw new Exception($"Cannot configure publisher for assembly {messageAssembly.GetName().Name} because it contains no types considered as events. Event types have to either implement NServiceBus_6.IEvent interface or match a defined event convention.");
            }
        ...
        public IEnumerable<PublisherTableEntry> GenerateWithoutBestPracticeEnforcement(Conventions conventions)
        {
            ...
            if (!entries.Any())
            {
                throw new Exception($"Cannot configure publisher for assembly {messageAssembly.GetName().Name} because it contains no types considered as messages. Message types have to either implement NServiceBus_6.IMessage interface or match a defined convention.");
            }
